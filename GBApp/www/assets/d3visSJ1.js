Shiny.addCustomMessageHandler('jsondata1', function(data) {
  // Parse the JSON data
  var d3flux001model = JSON.parse(data);
...
require([ "cola", "d3", "math", "FileSaver"], function ( cola,
  ...
  const container = d3.select("#d3flux001");
  ...
  container.select("svg").remove();
   ...
   d3.select("#d3flux001_options").append("style").text(`
        .zoomable {
      ...
      d3.select("#toggleZoomButton1")
        .on("click", function() {
          if (zoomEnabled) {
            // Disable zoom
            d3.select("#d3flux001 svg").on(".zoom", null);
            d3.select(this).text("Enable Zoom");
          } else {
            // Enable zoom
            d3.select("#d3flux001 svg").call(zoom);
...
d3.select("#d3flux001_options .svgbutton").on("click", function() { 
    ...
    var svgElement = d3.select("#d3flux001 svg").node(); 
