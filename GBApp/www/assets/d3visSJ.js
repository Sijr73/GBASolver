Shiny.addCustomMessageHandler('jsondata', function(data) {
  // Parse the JSON data
  var d3flux002model = JSON.parse(data);
  
  // Use the data to draw the D3 graph


  // Parse the JSON data
  //var d3flux002model = data;
...
require([ "cola", "d3", "math", "FileSaver"], function ( cola,
	d3, math, FileSaver) {
  ...
  const container = d3.select("#d3flux002");
  ...
  container.select("svg").remove();
    ...
    if (true) {
      var svg = d3.select("#d3flux002").append("svg")
        .attr("viewBox", "350 150 2000 300")
        .attr("style", "display:block;margin:auto;width:500%")
        .append("g");

       
        
    } else {
      var svg = d3.select("#d3flux002").select("svg");
    }
    
   d3.select("#d3flux002_options").append("style").text(`
        .zoomable {
          transition: transform 0.3s ease;
        }
      `);

      var zoomEnabled = false;  // Track the state of zoom
      ...
      // Use the existing button for toggling zoom
      d3.select("#toggleZoomButton")
        .on("click", function() {
          if (zoomEnabled) {
            // Disable zoom
            d3.select("#d3flux002 svg").on(".zoom", null);
            d3.select(this).text("Enable Zoom");
          } else {
            // Enable zoom
            d3.select("#d3flux002 svg").call(zoom);
            d3.select(this).text("Disable Zoom");
          }
          zoomEnabled = !zoomEnabled;  // Toggle the zoom state
        });
    ...
    svg.append("style").text(".node.rxn{fill:#ffa319}.node.rxn.hidden{fill:none;stroke:none;stroke-width:0}.link{fill:none;stroke-linecap:round}text.nodelabel{font-size:12pt;fill:#555;font-family:Arial}text.nodelabel.cofactor{font-size:9.600000000000001pt;fill:#789}text.nodelabel.rxn{fill:#a9a9a9;font-size:9.600000000000001pt;font-style:italic}marker.inactive,.node.inactive,text.inactive{fill-opacity:1.0}.link.inactive{stroke-opacity:1.0}");

    // Code for the figure manipulation buttons.
    d3.select("#d3flux002_options .reactionbutton").on("click", function() {
...
d3.select("#d3flux002_options .svgbutton").on("click", function() { 
    // Select the SVG element
    var svgElement = d3.select("#d3flux002 svg").node(); 
    ...
    svg.append("defs").selectAll("marker")
      ...
      .enter()
      .append("marker")
      .attr("id", function (d) { return "d3flux002" + d.id; })
    ...
    svg.append("defs").selectAll("marker")
      ...
      .enter()
      .append("marker")
      .attr("id", function (d) { return "d3flux002" + d.id + "_rev"; })
      .attr("viewBox", "0 0 10 10")
      .attr("refX", 9)
      .attr("refY", 5)
      .attr("markerUnits", "userSpaceOnUse")
    ...
    var link = svg.append('g').selectAll(".link")
      ...
      .append("path")
      .attr("class", function (d) {
        var labels = "link d3flux002" + d.rxn.id;
  ...
  // a particular direction
    ...
    d3.select("#d3flux002_options .download")
      .on("click", function () {
